...
#
# NOTE: LlamaIndex PGVectorStore defaults to a `json` (not `jsonb`) metadata_
# column (use_jsonb=False), so `jsonb_set` cannot be applied directly. Cast the
# value to jsonb for the update, then back to json for assignment.
BACKFILL_SQL = r"""
DO $$
BEGIN
  IF to_regclass('public.data_rag_docs') IS NOT NULL THEN
    ...
    SET metadata_ = jsonb_set(metadata_::jsonb, '{demo_token}', '"__default__"')::json
